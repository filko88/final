...
export function WelcomeModal() {
  ...
  const AgentLogo = ({
    ...
    const [src, setSrc] = useState(`/images/agents/${agent}.svg`)
    const [showLabel, setShowLabel] = useState(false)

    if (showLabel) {
      return <span className="text-[12px] leading-tight text-black">{label}</span>
    }
    ...
    return (
      <img
        ...
        onError={() => {
          if (src.endsWith(".svg")) {
            setSrc(`/images/agents/${agent}.webp`)
          } else {
            setShowLabel(true)
          }
        }}
                ...
                <div className="space-y-4 rounded-2xl border border-zinc-100 bg-zinc-50/50 p-4">
                  ...
                  >
                    <div className="relative z-10 flex items-center justify-between gap-2">
                      <AgentLogo agent="oopbuy" label="Oopbuy" sizeClass="h-10 w-28" />
                        ...
                        return (
                          ...
                          >
                            <div className="relative z-10 flex items-center justify-between gap-2 text-sm font-medium">
                              <AgentLogo agent={agent.value} label={agent.label} sizeClass="h-9 w-24" />
                            </div>
                          </Button>
                        )
                      })}
