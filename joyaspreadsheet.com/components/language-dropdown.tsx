...
const languageOptions = [
  { value: "en" as Language, label: "English", emoji: "🇺🇸" },
]
...
export function LanguageDropdown() {
  ...
  const getLanguageDisplayName = (lang: Language) => {
    ...
    return option?.label || lang.toUpperCase()
  }

  const getLanguageEmoji = (lang: Language) => {
    const option = languageOptions.find(l => l.value === lang)
    return option?.emoji || "🌐"
  }
      ...
      <PopoverTrigger asChild>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm">{getLanguageEmoji(language)}</span>
          ...
          {languageOptions.map((languageOption) => {
            ...
            return (
              <Button
                ...
                className={`w-full justify-start h-8 px-2 text-sm relative z-10 rounded transition-all duration-150 focus:outline-none focus:ring-0 ${isSelected
              ...
              >
                <span className="mr-2">{languageOption.emoji}</span>
                <span className="text-sm">{languageOption.label}</span>
              </Button>
            )
          })}
        </div>
